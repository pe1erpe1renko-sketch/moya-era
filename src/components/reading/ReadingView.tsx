...
function buildResearch(typeSlug: string, myDate: string, go: (path: string) => void): (sectionId: string) => ResearchCardProps | null {
  ...
  return (sectionId) => {
    switch (sectionId) {
      ...
      case "personality":
      case "health":
        return linkTo("/nastavnik", "Спросить наставника", "Наставник видит ваши числа и отвечает на вопрос, а не на аркан. Новому аккаунту — кредиты в подарок.", "Задать вопрос");
      case "purpose":
