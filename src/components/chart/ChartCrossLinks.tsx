...
/**
 * Перелинковка страниц по дате.
 *
 * Она нужна не для красоты: тридцать тысяч страниц на систему поисковик
 * обойдёт по ссылкам быстрее и охотнее, чем по одной карте сайта. Плюс
 * читателю логично посмотреть ту же дату в другой системе.
 */
  ...
  return (
    <nav className="mt-12 border-t border-border pt-8" aria-label="Другие разборы этой даты">
      ...
      <div className="mt-6 flex flex-wrap" style={{ gap: 10 }}>
        {inRange(prev) && (
          <Link href={chartPath(system, prev)} className={linkClass} style={{ fontSize: 15 }}>
            ← {formatBirthDate(prev)}
          </Link>
        )}
        {inRange(next) && (
          <Link href={chartPath(system, next)} className={linkClass} style={{ fontSize: 15 }}>
            {formatBirthDate(next)} →
          </Link>
        )}
        <Link href={`/${CHART_SYSTEMS[system].slug}`} className={linkClass} style={{ fontSize: 15 }}>
