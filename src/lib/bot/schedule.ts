...
/**
 * ЧАС ЕДИНСТВЕННОГО ЗАПУСКА В СУТКИ, UTC.
 *
 * Бесплатный тариф Vercel принимает только ежедневные расписания, и
 * любое чаще — например «каждые десять минут» — роняет деплой целиком:
 * сайт перестаёт обновляться, а не только рассылка. Поэтому в
 * `vercel.json` стоит один запуск в сутки, и этот час должен быть таким,
 * чтобы догоняющее правило («уже восемь по местным часам») сработало во
 * ВСЕХ поясах из списка бота за один проход. Самый западный — Берлин,
 * зимой UTC+1: в 07:00 UTC там ровно восемь. Раньше нельзя: в 06:00 UTC
 * Берлин и Калининград зимой ещё не проснулись, правило скажет «рано», а
 * следующий запуск только завтра — такие люди не получили бы сводку
 * никогда. По Москве это десять утра, на Камчатке — семь вечера. На
 * тарифе Pro расписание возвращается к «каждые десять минут» (см.
 * docs/BOT.md), и всем приходит в восемь по своим часам.
 */
export const DAILY_RUN_UTC_HOUR = 7;
