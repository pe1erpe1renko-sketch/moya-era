...
describe("неудача не отключает сразу", () => {
  ...
  it("в сообщении о неудаче не пугаем и не отбираем", () => {
    ...
    assert.ok(/бесплатные части разборов остались/.test(text) && /платные закрылись/.test(text), "надо честно сказать, что осталось, а что закрылось");
    assert.ok(/никуда не делись/.test(text), "накопленное остаётся");
