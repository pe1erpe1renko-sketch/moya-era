...
const ABOUT_PARAGRAPHS = [
  ...
  "Матрица не говорит, что случится. Она описывает устройство: какие качества заложены сильнее, какие слабее, где ваша опора, а где место, которое стоит замечать.",
  "Расчёт целиком арифметический. Одна и та же дата всегда даёт одну и ту же матрицу — это не интерпретация, а сложение.",
];

const SAMPLE_PARAGRAPHS = [
  "Центральный аркан 5 ставит вас в положение человека, которому нужна не подсказка, а устройство. Вы плохо переносите ответы без объяснения: даже верный совет, поданный как готовый вывод, вызывает сопротивление. Зато любая система, в которой видно, как одно следует из другого, схватывается быстро и надолго.",
  "В отношениях это проявляется мягче, чем можно ожидать. Вам важно не согласие, а понятность: почему человек поступает так, откуда у него это взялось, по каким правилам он живёт. Партнёр, который не может объяснить себя, утомляет сильнее, чем партнёр, который открыто не согласен.",
  "В работе пятый аркан почти всегда выводит к роли объясняющего. Это не обязательно преподавание — чаще это позиция человека, к которому приходят разобраться. Со временем возникает побочный эффект, о котором редко предупреждают: чем лучше вы объясняете чужие системы, тем",
];
...
const FAQ = [
  ...
  },
  {
    ...
    a: "Нет. Для матрицы достаточно даты. Время и место понадобятся, если захотите посмотреть натальную карту или дизайн человека.",
  },
  {
  ...
  },
  {
    ...
    a: "Нет. Она описывает качества, склонности и повторяющиеся сценарии. Что с этим делать — остаётся за вами.",
  },
];
...
export default function MatricaSudbyPage() {
  return (
    <DirectionPage<MatrixResult>
      ...
      resultLabel="Ваш центральный аркан"
      ...
      faq={FAQ}
      finalTitle="Посчитайте свою матрицу"
      ...
      explainBlock={(ctx) => (
        <ExampleScheme
          ...
          showButton={false}
          title={ctx ? "Откуда взялось ваше число" : "Каждое число можно проверить"}
          subtitle={
            ctx
              ? "Ваша дата, разложенная по шагам. Наведите на любое число, чтобы увидеть, из чего оно посчитано"
              : "Матрица — это арифметика. Наведите на любое число, чтобы увидеть, из чего оно посчитано"
          }
        />
      )}
    />
