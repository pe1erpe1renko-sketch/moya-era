...
const ABOUT_PARAGRAPHS = [
  ...
  "Каждый центр отвечает за свою функцию: один за то, как вы думаете, другой за то, как проявляетесь вовне, третий за жизненную энергию. Часть центров у человека определена, часть нет — и это главное, что читается по схеме.",
  "Определённый центр работает одинаково всегда, независимо от окружения: это то, на что можно опереться. Неопределённый усиливает то, что приходит от людей рядом, — отсюда ощущение, что в одной компании вы один человек, а в другой совсем другой.",
...
const SAMPLE_PARAGRAPHS = [
  "Проектор устроен так, что не производит энергию сам, а направляет чужую. Отсюда главная сложность: работать наравне с генераторами вы можете, но недолго, и расплата приходит не в тот же день, а через неделю усталости, которую не с чем связать.",
  "Стратегия проектора — ждать приглашения. Звучит пассивно, а на деле означает другое: ваша работа видна и востребована ровно тогда, когда о ней попросили. Инициатива без запроса чаще всего упирается в вежливое согласие и ничем не заканчивается.",
  "Эмоциональный авторитет добавляет к этому вторую задачу. Решение, принятое на подъёме, и решение, принятое на спаде, у вас будут разными — и оба покажутся верными в момент. Отсюда единственное правило, которое действительно меняет качество выбора, и оно про то,",
];
...
const HD_CENTERS = [
  ...
  },
  {
    id: "ajna",
    title: "Аджна",
    text: "Как вы обрабатываете информацию и приходите к выводам. Определённая аджна держится за свой способ думать, неопределённая гибко примеряет чужие и потому кажется себе непоследовательной",
  },
  {
    id: "throat",
    title: "Горло",
    text: "Выражение и действие: как вы проявляетесь вовне. Самый загруженный центр в схеме — сюда стекается почти всё остальное, и здесь замысел превращается в слово или поступок",
  },
  {
    id: "g",
    title: "Самость",
    text: "Направление и ощущение себя: кто вы и куда идёте. Определённая самость держит курс изнутри, неопределённая находит его через место и людей, среди которых оказывается",
  },
  {
    id: "heart",
    title: "Воля",
    ...
    <>
      ...
      {variation.preliminary ? (
        ...
        <p className="mt-4 text-text-secondary" style={{ fontSize: "clamp(14px, 1.05vw, 16px)", lineHeight: 1.6 }}>
          Разбор — ниже. Тип и стратегия читаются бесплатно, авторитет, профиль, каналы и ворота открывает подписка — одна на все шесть систем
        </p>
      )}
    </>
  );
    ...
    >
      <div className="mx-auto w-full max-w-[1240px] px-[4vw] md:px-6">
        ...
        <div className="mx-auto w-full" style={{ maxWidth: 720, marginTop: "clamp(48px, 6vw, 80px)" }}>
          ...
          >
            Первая карта отвечает за то, что вы осознаёте в себе, вторая — за то, что работает без
            вашего участия и обычно заметнее со стороны. Бодиграф получается наложением обеих,
