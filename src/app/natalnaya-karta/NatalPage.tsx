...
const SAMPLE_PARAGRAPHS = [
  "Солнце во Льве даёт потребность в отклике — не в аплодисментах, а в подтверждении, что сделанное замечено. Вы плохо переносите работу в стол и молчание в ответ. Это не тщеславие: без обратной связи у вас буквально падает мотивация, и никакая дисциплина этого не компенсирует.",
  "Луна в Деве при этом устроена противоположно. Она успокаивается порядком, деталями и предсказуемостью — тем, что Солнцу во Льве кажется мелким и скучным. Внутри вас постоянно спорят двое: один хочет размаха, другой сначала проверит расчёты.",
  "Именно на этом сочетании стоит задержаться. Оно объясняет, почему вы берётесь за крупное, а застреваете на мелочах, и почему отдых у вас выглядит странно для окружающих. Обычно люди с такой парой светил приходят к одному и тому же решению, и оно связано с тем,",
];
